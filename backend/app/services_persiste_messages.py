...
def get_previous_messages(key: str):
    ...
    for message in all_messages:
        if message["type"] == "system":
            formatted_messages.append(
                {"role": message["type"], "content": message["message"]})
    ...
    if 2 <= total_interactions <= 5:
        formatted_messages += [
            msg for interaction in interactions for msg in format_interaction(interaction)]
        if last_message:
            formatted_messages.append(last_message)
        return formatted_messages
    ...
    formatted_messages += format_interaction(
        first_interaction) if first_interaction else []
    formatted_messages += [
        msg for interaction in last_five_interactions for msg in format_interaction(interaction)]
